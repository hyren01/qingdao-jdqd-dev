...
def rule201(sentence, sub_sentences):
    # 匹配模式: 如果..., ...
    match_mode = 'full'
    return pattern.rule_kscs(sentence, sub_sentences, keywords_if, parts, match_mode)


def rule301(sentence, sub_sentences):
    # 匹配模式: ..., 如果...的话.
    keywords = list(product(if_words, ['的话']))
    return pattern.rule_scksk(sentence, sub_sentences, keywords, parts)


def rule302(sentence, sub_sentences):
    # 匹配模式: ..., 如果...
    match_mode = 'full'
    return pattern.rule_scks(sentence, sub_sentences, keywords_if, parts, match_mode)
