...
def rule101(sentence, sub_sentences):
    ...
    return pattern.rule_ksckscas(sentence, sub_sentences, keyword_pairs, parts)


def rule102(sentence, sub_sentences):
    # 匹配模式: 因为..., 所以...
    keyword_pairs = list(product(cause_conjs, effect_conjs))
    return pattern.rule_kscks(sentence, sub_sentences, keyword_pairs, parts, match_mode='long')


def rule2(sentence, sub_sentences):
    # 匹配模式: ...之所以..., 是因为...
    keyword_pairs = product(['之所以'], ['是' + c for c in cause_conjs])
    return pattern.rule_skscks(sentence, sub_sentences, keyword_pairs, parts, reverse=True)
...
def rule3(sentence, sub_sentences):
    ...
    return pattern.rule_sks(sentence, sub_sentences, keywords, parts, reverse=True)
...
def rule401(sentence, sub_sentences):
    # 匹配模式: ...因为..., ...
    return pattern.rule_skscs(sentence, sub_sentences, keywords_cause, parts)


def rule402(sentence, sub_sentences):
    # 匹配模式: 因为..., ...
    return pattern.rule_kscs(sentence, sub_sentences, keywords_cause, parts)


def rule5(sentence, sub_sentences):
    # 匹配模式: ..., 因为...
    return pattern.rule_scks(sentence, sub_sentences, keywords_cause, parts, match_mode='full')
...
def rule6(sentence, sub_sentences):
    ...
    return pattern.rule_sksk(sentence, sub_sentences, keywords, parts, match_mode='long')


def rule7(sentence, sub_sentences):
    # 匹配模式:..., 导致...
    keywords = [[w] for w in effect_verbs]
    return pattern.rule_scks(sentence, sub_sentences, keywords, parts, match_mode='full')


def rule702(sentence, sub_sentences):
    # 匹配模式:...导致...
    keywords = [[w] for w in effect_verbs]
    return pattern.rule_sks(sentence, sub_sentences, keywords, parts)
...
def rule8(sentence, sub_sentences):
    ...
    return pattern.rule_sksk(sentence, sub_sentences, keyword_pairs, parts)
