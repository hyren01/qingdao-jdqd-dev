...
def rule101(sentence, sub_sentences):
    # 匹配模式: ..., 是一种...
    return pattern.rule_scks(sentence, sub_sentences, keywords_is, parts)


def rule102(sentence, sub_sentences):
    # 匹配模式: ...是一种...
    return pattern.rule_sks(sentence, sub_sentences, keywords_is, parts)


def rule201(sentence, sub_sentences):
    # 匹配模式: 包括...在内的...
    keywords = list(product(contain_words, ['在内的']))
    return pattern.rule_kscks(sentence, sub_sentences, keywords, parts, comma=False)


def rule3(sentence, sub_sentences):
    # 匹配模式: ..., 包括...等
    keywords = list(product(contain_words, ['等']))
    return pattern.rule_scksk(sentence, sub_sentences, keywords, parts, reverse=True)
