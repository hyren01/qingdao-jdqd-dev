    ...
    def get_text(self):
        ...
        for child in self.children:
            ...
            if isinstance(child, NounPhrase):
                ...
                text += value[0]
                PPcodes += value[1]
            if child.label[:2] in ["JJ", "NN", "NR", "DT"]:
                text += " " + child.text
        return text, PPcodes
...
# --          print('cp-entry')
        ...
        if True:
            if self.children[0].label in ["VBD", "VBN"]:
                level = self.parent
                if level.label == "NP": # or level.label == "QP" or level.label == "DNP"
                    self.passive = True
                    return True
                for i in range(2):
                    if level and isinstance(level, VerbPhrase):
                        if level.children[0].text in [
                                "AM", "IS", "ARE", "WAS", "WERE", "BE", "BEEN", "BEING", "是"]:
                            self.passive = True
                            return True
                    level = level.parent
                    if not level:
        ...
        def match_noun(path, phrase=self if not self.check_passive()
            ...
            if preplimit:
                ...
                for child in phrase.children:
                    if child.label in ("NP", "ADVP"):   #, "QP", "DNP"
...
#--         print('si:==>',self.tree.get_parse_string())  # -- debug
    ...
    def str_to_tree(self, str):
        ...
        for element in segs[1:]:
            if element.startswith("("):
                lab = element[1:]
                if lab == "NP":      # 增加了=="QP"、"DNP"判断     or lab == "QP" or lab == "DNP"
    ...
    def get_metadata(self, entry):
        ...
        while id(next) in metadict:
            ...
            if store[1] != 0:     # 自行添加，若未匹配到事件编码，则不记录该动词
