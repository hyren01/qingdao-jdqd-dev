        ...
        for event, elem in tree:
            if event == "end" and elem.tag == "Sentence":
                ...
                if parsed:
                    ...
                    content_spo = story.find('Spo').text    # 自己加入的，主谓宾数据
                    content_namedentity = story.find('Namedentity').text    # 自己加入的，命名实体识别数据
                    words = story.find('Words').text    # 自己加入的，单词数据
                else:
                    parsed_content = ''
                    content_spo = ''
                    content_namedentity = ''
                ...
                if story.attrib['sentence'] == 'True':
                    ...
                    text = text.replace('\n', ' ').replace('  ', ' ')
                    sent_dict = {'content': text, 'parsed': parsed_content, 'spo': content_spo,
                                 'namedentity': content_namedentity, 'words': words}
                    meta_content = {'date': story.attrib['date'],
                                    'source': story.attrib['source'],
                                    'source_file': story.attrib['source_file']}
                    ...
                    meta_content = {'date': story.attrib['date'], 'source_file': story.attrib['source_file']}
...
def read_input_by_data(id, sentence_txt, constituency_parse, spo):

    if id is None:
        print('Need to properly format your input in...')
        return {}

    parsed_content = constituency_parse
    parsed_content = utilities._format_parsed_str(parsed_content)
    content_spo = spo  # 自己加入的，主谓宾数据
    # content_namedentity = namedentity  # 自己加入的，命名实体识别数据
    # words = words  # 自己加入的，单词数据

    holding = {}
    # if sentence == 'True':
    entry_id, sent_id = id.split('_')

    text = sentence_txt
    text = text.replace('\n', ' ').replace('  ', ' ')
    sent_dict = {'parsed': parsed_content, 'spo': content_spo, 'content': text}
    meta_content = {'date': '', 'source': ''}
    content_dict = {'sents': {sent_id: sent_dict}, 'meta': meta_content}
    # else:
    #     entry_id = id
    #
    #     text = sentence_txt
...
def read_input_by_constituency(content, constituency_parse):

    parsed_content = utilities._format_parsed_str(constituency_parse)

    holding = {}

    sent_dict = {'parsed': parsed_content, 'content': content}

    content_dict = {'sents': {"111": sent_dict}, 'meta': {'date': '20000220', 'source': ''}}

    holding["110"] = content_dict
