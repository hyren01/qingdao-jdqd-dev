    ...
    for key, val in sorted(event_dict.items()):
        ...
        for sent in val['sents']:
            ...
            if 'parsed' in event_dict[key]['sents'][sent]:
                ...
                if coded_events is None:    # 此处因为使用了newdict.txt导致了一个运行时错误，增加此处来解决该问题
                    continue
...
# input_event_xlsx_path = Config().input_event_xlsx_path
...
def run(filepaths, out_file, s_parsed):
    ...
    if PETRglobals.NullVerbs:
        ...
        # PETRwriter.write_events(updated_events, 'evts.' + out_file, event_xlsx_path)
        # PETRwriter.write_events(updated_events, out_file, input_event_xlsx_path)
        pass


def run_by_data(id, sentence_txt, constituency_parse, spo):
    # this is the routine called from main()
    events = PETRreader.read_input_by_data(id, sentence_txt, constituency_parse, spo)
    updated_events = do_coding(events)
    return PETRwriter.write_events(updated_events)


def run_by_constituency(content, constituency):
    events = PETRreader.read_input_by_constituency(content, constituency)
    updated_events = do_coding(events)
    return PETRwriter.get_event_code(updated_events)
