...
from services import PETRglobals
...
def story_filter(story_dict, story_id):
    ...
    # source_file = story_dict['meta']['source_file']     # 自行添加
        ...
        if 'events' in sent_dict:
            ...
            for event in story_dict['sents'][sent]['events']:
                ...
                try:
                    ...
                    # filtered[event_tuple]['source_file'] = source_file
                    filtered[event_tuple]['content'] = sent_dict['content']
                    # filtered[event_tuple]['words'] = sent_dict['words']
                    # filtered[event_tuple]['namedentity'] = sent_dict['namedentity']
                    filtered[event_tuple]['spo'] = sent_dict['spo']
...
def story_filter_by_constituency(story_dict, story_id):
    """
    One-a-story filter for the events. There can only be only one unique
    (DATE, SRC, TGT, EVENT) tuple per story.
