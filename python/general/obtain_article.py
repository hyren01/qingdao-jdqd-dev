...
def get_articles_from_db(num=None):
    ...
    t1 = time.time()
    if num:
        sql = f"select * from t_article_msg_zh where article_id not in ( select distinct article_id from ebm_event_sentence) and content <>'' limit {num}"
    else:
        sql = "select * from t_article_msg_zh where article_id not in ( select distinct article_id from ebm_event_sentence) and content <>''"
    cursor = conn.cursor()
    cursor.execute(sql)
