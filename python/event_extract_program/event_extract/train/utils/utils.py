...
def generate_trained_model_path(trained_model_dir, trained_model_name):
    """
    传入模型保存文件夹，和训练后模型名称，按照日期生成模型保存文件夹，并生成模型保存路径，返回
    :param trained_model_dir: (str)模型保存文件夹
    :param trained_model_name: (str)模型名称
    :return: trained_model_path(str)模型保存路径
    :raise:ValueError
    """
    if os.path.isfile(trained_model_dir) or not os.path.exists(trained_model_dir):
        LOG.info(f"{trained_model_dir} is wrong!")
        raise ValueError
    if not trained_model_name.endswith(".h5"):
        LOG.error(f"{trained_model_name} must end with .h5")
        raise ValueError

    trained_model_dir = os.path.join(trained_model_dir, time.strftime('%Y-%m-%d', time.localtime(time.time())))
