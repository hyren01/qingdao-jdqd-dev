...
def rule101(sentence, sub_sentences):
    # 匹配模式: ...只要..., ...就...
    return pattern.rule_skscsks(sentence, sub_sentences, keywords1)


def rule102(sentence, sub_sentences):
    # 匹配模式: 只要..., ...就...
    return pattern.rule_kscsks(sentence, sub_sentences, keywords1)


def rule103(sentence, sub_sentences):
    # 匹配模式: 只要..., 就...
    return pattern.rule_kscks(sentence, sub_sentences, keywords1)


def rule201(sentence, sub_sentences):
    # 匹配模式: ...只有..., ...才能...
    return pattern.rule_skscsks(sentence, sub_sentences, keywords2)


def rule202(sentence, sub_sentences):
    # 匹配模式: ...只有..., 才能...
    return pattern.rule_skscks(sentence, sub_sentences, keywords2)


def rule203(sentence, sub_sentences):
    # 匹配模式: 只有..., 才能...
    return pattern.rule_kscks(sentence, sub_sentences, keywords2)
