...
def rule101(sentence, sub_sentences):
    # 匹配模式: ...一边..., 一边...
    match_mode = 'short'
    return pattern.rule_skscks(sentence, sub_sentences, parallel_pairs, match_mode)


def rule102(sentence, sub_sentences):
    # 匹配模式: ...一边...一边...
    match_mode = 'short'
    return pattern.rule_skscks(sentence, sub_sentences, parallel_pairs, match_mode, comma=False)


def rule103(sentence, sub_sentences):
    # 匹配模式: 一边...一边...
    match_mode = 'short'
    return pattern.rule_kscks(sentence, sub_sentences, parallel_pairs, match_mode)


def rule104(sentence, sub_sentences):
    # 匹配模式: 一边...一边...
    match_mode = 'short'
    return pattern.rule_kscks(sentence, sub_sentences, parallel_pairs, match_mode, comma=False)
...
def rule202(sentence, sub_sentences):
    # 匹配模式: ..., 同时...
    return pattern.rule_sckcs(sentence, sub_sentences, parallel_pairs, comma2=False)


def rule203(sentence, sub_sentences):
    # 匹配模式: ...同时, ...
    return pattern.rule_sckcs(sentence, sub_sentences, parallel_pairs, comma1=False)


def rule204(sentence, sub_sentences):
    # 匹配模式: ...同时 ...
    return pattern.rule_sckcs(sentence, sub_sentences, parallel_pairs, comma1=False, comma2=False)
