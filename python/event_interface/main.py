...
from helper import event_helper
from config.config import Config
from utils.data_util import clear_web_data

import services.graph_database as graph_service
import services.rdms_database as rdms_service
...
def get_event_rel_by_id():
    ...
    try:
        ...
        graph_db = get_gdb()
        result = graph_service.get_event_rel_by_id(graph_db, event_id, event_tag)

        return Response(json.dumps({"status": "success", "data": result}), mimetype="application/json")
...
def event_parsed_extract():
    ...
    try:
        ...
        success, article_id = event_helper.event_parsed_extract_helper(rdms_db, graph_db, fulltext_db, content,
                                                                       content_id, event_tag, update_synonyms)
        if success:
            success = "success"
        else:
            success = "error"
        ...
        return {"status": success, "content_id": article_id}
...
def getEventRel():
    try:
        ...
        rdms_db = get_db()
        success, result_event_rel = event_helper.get_event_rel(rdms_db, event_id)

        # 14、接口返回内容id（文章或者句子的id）。
        return Response(json.dumps({"status": success, "result": result_event_rel}), mimetype="application/json")
...
def getArticleInfo():
    try:
        ...
        rdms_db = get_db()
        success, result_article = event_helper.get_article_info(rdms_db, article_id)
...
def getEvent2event():
    try:
        ...
        graph_db = get_gdb()
        success, result, result_d = event_helper.get_event_to_event(graph_db, event_source_sentence,
                                                                    event_target_sentence)

        # 14、接口返回内容id（文章或者句子的id）。
        return Response(json.dumps({"status": success, "data": result_d, "link": result}), mimetype="application/json")
