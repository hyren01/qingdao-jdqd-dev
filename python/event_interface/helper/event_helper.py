    ...
    for sentence_parsed in sentence_parsed_array:
        ...
        for event in sentence_parsed["events"]:
            ...
            if check_event_info:
                ...
                event_id = __storage_new_event(rdms_db, subject, verb, object, short_sentence, cameo, triggerloc_index,
                                               sentiment_analysis, event_datetime, negative_word, state, event_location,
...
def event_parsed_extract_helper(rdms_db, graph_db, fulltext_db, content, content_id, event_tag, update_synonyms):
    ...
    # if not res_dict:
    content_id, event_id = __parsed_sentence_and_insert2db(rdms_db, graph_db, fulltext_db, content_id, content,
...
def get_event_rel(rdms_db, event_id):
    success, result_event_rel = rdms_service.get_event_rel_and_article(rdms_db, event_id)
    return success, result_event_rel


def get_article_info(rdms_db, article_id):
    success, result_article = rdms_service.get_article_info(rdms_db, article_id)
    return success, result_article


def get_event_to_event(graph_db, event_source_sentence, event_target_sentence):
    success, result, result_d = graph_service.get_event_to_event(graph_db, event_source_sentence, event_target_sentence)
    return success, result, result_d
